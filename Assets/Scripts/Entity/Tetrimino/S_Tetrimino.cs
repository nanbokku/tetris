...
{
    public override Data.BlockType BlockType { get { return Data.BlockType.S; } }
    ...
    private Vector3[,] rotCoodinate =
    {
        ...
        {new Vector3(-1f, 0.5f, -0.5f), new Vector3(0f, -0.5f, -0.5f), new Vector3(-1f, 1.5f, -0.5f), new Vector3(0f, 0.5f, -0.5f)}
    };
    ...
    {
        // 右端に達したとき左入力しか受け付けない
        if (rotation == Data.BlockRotation.Normal && transform.position.x >= 8.5f && direction > 0
            || rotation == Data.BlockRotation.Right && transform.position.x >= 9.5f && direction > 0) return;

        // 左端に達したとき右入力しか受け付けない
        if (transform.position.x <= 1.5f && direction < 0) return;
    ...
    {
        var iniPos = new Vector2Int[] { new Vector2Int(3, 18), new Vector2Int(4, 18), new Vector2Int(5, 19), new Vector2Int(4, 19) };
